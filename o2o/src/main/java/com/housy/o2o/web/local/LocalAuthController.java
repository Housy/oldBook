package com.housy.o2o.web.local;
...
public class LocalAuthController {
	...
	private Map<String, Object> bindLocalAuth(HttpServletRequest request) {
		...
		//从session中获取当前用户信息（用户一旦铜鼓微信登录之后，便能获取用户信息
		...
		if (userName != null && password != null && user != null && user.getUserId() != null) {
			...
			//绑定账号
			LocalAuthExecution ls = localAuthService.bindLocalAuth(localAuth);
			if (ls.getState() == LocalAuthStateEnum.SUCCESS.getState()) {
				modelMap.put("success", true);
			} else {
				modelMap.put("success", false);
				modelMap.put("errMsg", ls.getStateInfo());
			}
		} else {
			modelMap.put("success", false);
