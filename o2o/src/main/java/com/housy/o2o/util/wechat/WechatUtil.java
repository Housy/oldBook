package com.housy.o2o.util.wechat;
...
import com.housy.o2o.dto.UserAccessToken;
import com.housy.o2o.dto.WechatUser;
...
public class WechatUtil {
	 ...
	 * @throws IOException
	 */
//	@GetMapping("user/{id}") @PathVariable
//	localhost:100/api/user?id=xx @RequestParam
//	@GetMapping("user")
	public static UserAccessToken getUserAccessToken(String code) throws IOException {
		// 测试号信息里的appId
		String appId = "wx3c1387bb01708686";
		log.debug("appId:" + appId);
		// 测试号信息里的appsecret
		String appsecret = "a69a07647045ced27a69e6ac98c77620";
		...
		try {
			...
			log.error("获取用户accessToken失败JsonParseException: " + e.getMessage());
			e.printStackTrace();
		} catch (JsonMappingException e) {
			log.error("获取用户accessToken失败JsonMappingException: " + e.getMessage());
			e.printStackTrace();
		} catch (IOException e) {
			log.error("获取用户accessToken失败IOException: " + e.getMessage());
			e.printStackTrace();
		}
		if (token == null) {
	...
	public static WechatUser getUserInfo(String accessToken, String openId) {
		...
		try {
			...
			log.error("获取用户信息失败JsonParseException: " + e.getMessage());
			e.printStackTrace();
		} catch (JsonMappingException e) {
			log.error("获取用户信息失败JsonMappingException: " + e.getMessage());
			e.printStackTrace();
		} catch (IOException e) {
			log.error("获取用户信息失败IOException: " + e.getMessage());
			e.printStackTrace();
		}
		if (user == null) {
			log.error("获取用户信息失败。");
